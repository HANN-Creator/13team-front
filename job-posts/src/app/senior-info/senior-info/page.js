...
export default function SeniorInformationPage() {
  ...
  const handleNext = (event) => {
    ...
    else router.push('/senior-info/senior-more-info');
  };
    ...
    <div className="min-h-screen bg-gray-50 px-6 py-4">
      ...
      <form onSubmit={handleNext}>
            ...
            {['여성', '남성'].map((_gender, index) => (
              <button
                ...
                className={`w-[calc(50%-0.75rem)] py-4 rounded-[61px] border ${
                  gender === _gender ? 'bg-orange bg-opacity-10 border-orange text-orange' : 'bg-white text-gray-600'
                }`}
        ...
        <div className="mt-12">
          ...
          <div className="mt-2 flex flex-wrap gap-3 text-base font-semibold">
            {grades.map((_grade, index) => (
              <button
                type='button'
                key={index}
                className={`w-[calc(50%-0.75rem)] py-4 rounded-[61px] border ${
                  grade === _grade ? 'bg-orange bg-opacity-10 border-orange text-orange' : 'bg-white text-gray-600'
                }`}
                onClick={() => setGrade(_grade)}
              >
                {_grade}
              </button>
            ))}
            ...
            {dementiaInfoList.map((symptom, index) => (
              <button
                ...
                className={`w-[calc(50%-0.75rem)] h-[76px] py-1 px-3 rounded-[61px] border ${
                  dementiaInfo.includes(symptom) ? 'bg-orange bg-opacity-10 border-orange text-orange' : 'bg-white text-gray-600'
                }`}
            ...
            {cohabitantList.map((_cohabitant, index) => (
              <button
                ...
                className={`w-full py-4 rounded-[61px] border ${
                  cohabitant === _cohabitant ? 'bg-orange bg-opacity-10 border-orange text-orange' : 'bg-white text-gray-600'
                }`}
        ...
        <button
          type="submit"
          className="w-full bg-orange text-white py-3 rounded-[9px] text-base font-semibold mx-1 mt-12"
